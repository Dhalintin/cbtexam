...
@section('space-work')
    ...
    <div class="inline">
        <div x-data="{ isOpen: false, formData: {}, uploadModal: false, uploadSubModal: false }" setActiveLink(2) x-on:keyup.esc="isOpen = false, uploadModal = false, uploadSubModal:false" x-on:clickaway="isOpen = false, uploadModal = false, uploadSubModal:false" class="inline">
            ...
            <button @click="uploadModal = true" class="text-white border bg-[#1A988A] border-green-200 px-4 py-2 rounded-md hover:bg-amber-200 hover:text-black transition inline">
                Upload Objective Questions
            </button>
    
            @include('layout/modals/uploadObjquestion')
        </div>
        <div x-data="{ uploadSubModal: false }" class="inline">
            <button @click="uploadSubModal = true; formData = { id: '{{ Route('uploadQuestion', 1) }}'}" class="text-white border bg-[#1A988A] border-green-200 px-4 py-2 rounded-md hover:bg-amber-200 hover:text-black transition inline">
        ...
        <div x-data="{ isOpen: false, formData: {}, answermodal:false }" x-on:keyup.esc="isOpen = false" class="min-h-screen">
            <table class="w-full text-left ">
                <thead class="border-t-4 border-b-4 text-lg font-bold text-[#103a36]">
                    <th scope="col" class="pt-2 pb-3 mb-6  pr-5">Question</th>
                ...
                <tbody>
                    @if (count($questions) > 0)
                        @foreach ($questions as $question)
                            <tr class="border-t-2">
                                <td class="pt-3 pb-2">{{ $question->question }}</td>
